<?php
/*
 * filemgr.php - a simple Web-based file manager
...
?>
	...
	<table class='status' cellspacing="0" cellpadding="0"><tr class='odd'><td class='check'>
	<table class='status' cellspacing="1" cellpadding="4">
	<tr class='odd'>
		<td bgcolor="#EEEEEE" align="center"><b><?php echo(htmlentities($_SERVER['SERVER_ADDR'])); ?></b></td>
...
<tr class='odd'>
	...
	<table class='status' cellspacing="1" cellpadding="4">
<tr class='odd'>
		<td bgcolor="#EEEEFF" align="center"><textarea name="content" rows="<?php echo($editrows); ?>" cols="<?php echo($editcols); ?>" wrap="off"><?php
...
function show ($file) {
	...
	include('header.php');
	echo '<textarea name="show" rows="25" cols="90" wrap="off">';
...
function show_highlight ($file) {
	...
	if (@is_readable($file) && @is_file($file)) {
		...
		echo("</title></head>\n<body>\n<table cellpadding='4' class='status'>\n<tr class='odd'>\n<td>\n<code style='color: #999999'>\n");
...
<tr class='odd'>
		<td bgcolor="#EEEEEE" colspan="2">
			<table class='status' cellspacing="0" cellpadding="0">
			<tr class='odd'>
				<td class='value'><?php echo("<a href='$self?dir=" . urlencode($homedir) . "$p'>" . $words['dir']); ?></a>:&nbsp;</td>
...
<?php
	...
	echo("		<td class='value'><a href='$self?sort=filename&reverse=" . (($sort == 'filename') ? !$reverse : 0) . "&$p'><b>{$words['filename']}</b></a></td>\n");
	if ($GLOBALS['showsize']) echo("		<td class='value' align='right'><a href='$self?sort=size&reverse=" . (($sort == 'size') ? !$reverse : 0) . "&$p'><b>{$words['size']}</b></a></td>\n");
	echo("		<td class='value'><a href='$self?sort=permission&reverse=" . (($sort == 'permission') ? !$reverse : 0) . "&$p'><b>{$words['permission']}</b></a></td>\n");
	echo("		<td class='value'><a href='$self?sort=owner&reverse=" . (($sort == 'owner') ? !$reverse : 0) . "&$p'><b>{$words['owner']}</b></a></td>\n");
	echo("		<td class='value'><a href='$self?sort=group&reverse=" . (($sort == 'group') ? !$reverse : 0) . "&$p'><b>{$words['group']}</b></a></td>\n");
	echo("		<td  class='value'><b>{$words['functions']}</b></td>\n");
	echo("	</tr>\n");
	$p = '&' . 'SID';
	...
	foreach ($files as $file) {
		...
		if ($file['is_link']) {
			...
			echo("		<td class='value' align='center' width='20px'><img src='img/dir.png'></td>\n");
			echo("		<td$cspan class='value'>");
			...
			echo("		<td class='value' align='center' width='20px'><img src='img/");
			if (substr($file['filename'], 0, 1) == '.') echo('file.png'); else echo('file.png');
			echo("'></td>\n");
			echo('		<td');
			...
			if ($GLOBALS['showsize'] && $file['is_file']) {
				echo("		<td class='value' align='right' nowrap>");
				if ($file['is_file']) echo("{$file['size']} B");
				echo("</td>\n"); 
			}
		}
		echo('		<td class="value" class="perm">');
		...
		echo("		<td class='value' nowrap>$f</td>\n");
		echo("	</tr>\n");
...
?>
	<p>
	<table class='status' cellspacing="0" cellpadding="0"><tr class='odd'><td class='check'>
	<table class='status' cellspacing="1" cellpadding="4"><tr class='odd'><td class='check'>
	<?php echo("$str\n"); ?>
	</td></tr></table>
	</td></tr></table>
	</p>
